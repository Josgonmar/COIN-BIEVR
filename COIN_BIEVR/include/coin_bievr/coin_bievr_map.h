#ifndef COIN_BIEVR_MAP_H_
#define COIN_BIEVR_MAP_H_
...
// xyz + sensor range + normalized intensity.
using MapPoint = Eigen::Matrix<double, 5, 1>;

struct Voxel {
  bool observed_{false}; // We consider a voxel observed if we have seen enough
                         // points inside it
  ...
  // Raw points accumulated while the voxel is not yet observed (no normal yet,
  // so they cannot be projected into a bump image). Cleared once the voxel
  // becomes observed.
...
public:
  struct Config {
    ...
    double norm_tol_deg{
        3.0}; // if normal changes more than this, reproject bump image
  };

  explicit COINBIEVRMap(Config config);

  bool integratePoints(const IntensityPointcloud &input_cloud,
                       const std::vector<double> *ranges = nullptr);

  inline size_t hashIndex(const Point &point) const {
    Eigen::Vector3i voxel_idx =
        (point * inv_voxel_size_).array().floor().cast<int>();
...
private:
  ...
  // Voxel update / image pipeline, in the order integratePoints invokes them.
  bool updateNormal(Voxel &voxel);

  bool updateImages(const std::vector<MapPoint> &points, Voxel &voxel,
                    bool normal_change);

  ImageBounds computeImageSize(const Voxel &voxel,
                               const Point &reference_point) const;

  void reprojectImage(Voxel &voxel, const ImageBounds &bounds,
                      Eigen::MatrixXi &changed);

  void integratePoints(const std::vector<MapPoint> &points, Voxel &voxel,
                       Eigen::MatrixXi &changed);

  void dilateMask(const Eigen::MatrixXi &changed,
                  const Eigen::MatrixXf &weights,
                  Eigen::MatrixXi &changed_dilated);

  void maskedGaussianSmooth(const Eigen::MatrixXf &image,
                            const Eigen::MatrixXf &weights,
  ...
  double norm_tol_rad_{
      3.0}; // if normal changes more than this, update bump image
...
#endif // COIN_BIEVR_COIN_BIEVR_MAP_H_

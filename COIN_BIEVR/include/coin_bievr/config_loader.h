#ifndef COIN_BIEVR_CONFIG_LOADER_H_
#define COIN_BIEVR_CONFIG_LOADER_H_
...
//
// The YAML is organised into sections (topics / calibration / lidar / intensity /
// imu / map / preprocess / optimization / debug). Files are
...
inline void printConfigOverview(const Config& config) {
  ...
  os << "  intensity_voxels:     " << hc.preprocess.intensity_voxels << "\n";
  os << "  intensity_downsample_res_m: "
     << hc.preprocess.intensity_downsample_resolution << "\n";
  os << "intensity:\n";
  os << "  image_size:           " << hc.intensity.image_width << " x "
     << hc.intensity.image_height << "\n";
  os << "  vertical_fov_deg:     " << hc.intensity.vertical_fov_deg << "\n";
  os << "  brightness_window:    " << hc.intensity.brightness_window_width << " x "
     << hc.intensity.brightness_window_height << "\n";
...
inline bool loadConfigFromYaml(const std::vector<std::string>& yaml_paths, Config& config) {
  ...
  if (!config_internal::getPositive(yaml, "preprocess", "downsample_resolution_m", 0.15,
      ...
      !config_internal::getPositive(yaml, "preprocess", "intensity_downsample_resolution_m", 0.1,
                                    hc.preprocess.intensity_downsample_resolution) ||
      !config_internal::getPositive(yaml, "preprocess", "intensity_voxels", 100,
                                    intensity_voxels)) {
    return false;
  }
  hc.preprocess.intensity_voxels = static_cast<size_t>(intensity_voxels);
  hc.preprocess.informed_sampling = yaml.get<bool>("preprocess", "informed_sampling", false);

  // --- intensity preprocessing ---
  if (!config_internal::getPositive(yaml, "intensity", "image_width", 1024,
                                    hc.intensity.image_width) ||
      !config_internal::getPositive(yaml, "intensity", "image_height", 128,
                                    hc.intensity.image_height) ||
      !config_internal::getPositive(yaml, "intensity", "vertical_fov_deg", 90.0,
                                    hc.intensity.vertical_fov_deg) ||
      !config_internal::getPositive(yaml, "intensity", "brightness_window_width", 41,
                                    hc.intensity.brightness_window_width) ||
      !config_internal::getPositive(yaml, "intensity", "brightness_window_height", 7,
                                    hc.intensity.brightness_window_height) ||
      !config_internal::getPositive(yaml, "intensity", "output_scale", 140.0,
                                    hc.intensity.output_scale)) {
    return false;
  }
  if (hc.intensity.vertical_fov_deg > 180.0 ||
      hc.intensity.brightness_window_width > hc.intensity.image_width ||
      hc.intensity.brightness_window_height > hc.intensity.image_height ||
      hc.intensity.brightness_window_width % 2 == 0 ||
      hc.intensity.brightness_window_height % 2 == 0) {
    LOG(E, "Config error: intensity FOV must be <= 180 deg and brightness windows must be odd "
           "and fit inside the configured image.");
    return false;
  }
  ...
  // `coin_bievr_ascii.txt` next to the config files that were loaded. Those files may
  ...
  if (hc.print_dashboard && hc.dashboard_ascii_path.empty()) {
    ...
    for (const std::string& p : yaml_paths) {
      ...
      for (const fs::path& candidate_dir : {dir, dir.parent_path()}) {
        const fs::path candidate = candidate_dir / "coin_bievr_ascii.txt";
        std::error_code ec;
...
#endif  // COIN_BIEVR_CONFIG_LOADER_H_

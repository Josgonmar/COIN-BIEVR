#ifndef COIN_BIEVR_COMMON_H_
#define COIN_BIEVR_COMMON_H_
...
// xyz + normalized intensity (row 3), used by mapping, photometric sampling,
...
// Spatial cloud with a normalized intensity channel (row 3).
class IntensityPointcloud : public PointcloudBase<IntensityPoint> {
 public:
  ...
  // intensity is taken as a non-owning view and copied into the assembled cloud.
  ...
  IntensityPointcloud(const Pointcloud& points, const Intensities& intensities) {
    resize(points.size());
    data_.topRows(3) = points.data();
    data_.bottomRows(1) = intensities;
  }

...
COINBIEVRMap::COINBIEVRMap(Config config) : config_(config) {
...
bool COINBIEVRMap::integratePoints(const IntensityPointcloud& cloud,
  ...
  // Each entry is {voxel hash, point (xyz, sensor range, normalized intensity)}.
  ...
  tbb::parallel_for(tbb::blocked_range<size_t>(0, cloud.size()),
                    [&](const tbb::blocked_range<size_t>& r) {
                      for (size_t i = r.begin(); i != r.end(); ++i) {
                        hashed_points[i].second.head(3) = cloud[i].head<3>();
                        if (ranges) {
                          hashed_points[i].second(3) = (*ranges)[i];  // weight by range
                        } else {
                          hashed_points[i].second(3) = 1;
                        }
                        hashed_points[i].second(4) = cloud[i](3);
  ...
  tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<size_t>& r) {
        ...
        for (size_t i = r.begin(); i != r.end(); ++i) {
          ...
          updateImages(voxel_points, iter->second.voxel, normal_change);
        }
      });
  ...
  if (removal_counter > 0) {
    LOG(I, "COINBIEVRMap exceeded max_size. Removed " << removal_counter << " old voxels.");
  }
  return true;
}

bool COINBIEVRMap::updateNormal(Voxel& voxel) {
...
bool COINBIEVRMap::updateImages(const std::vector<MapPoint>& points, Voxel& voxel,
                                bool normal_change) {
  if (!voxel.observed_) {
    return false;
  }
  ...
  computeScore(voxel);
  computeIntensityInformation(voxel);

  return true;
}

COINBIEVRMap::ImageBounds COINBIEVRMap::computeImageSize(
    const Voxel& voxel, const Eigen::Vector3d& reference_point) const {
...
void COINBIEVRMap::reprojectImage(Voxel& voxel, const ImageBounds& bounds,
                                  Eigen::MatrixXi& changed) {
...
void COINBIEVRMap::integratePoints(const std::vector<MapPoint>& points, Voxel& voxel,
                                   Eigen::MatrixXi& changed) {
  for (const auto& p : points) {
    ...
    const double bump_old = voxel.bump_img_(y, x);
    const double intensity_old = voxel.intensity_img_(y, x);
    const double weight = voxel.bump_weights_(y, x);
    ...
    const double weight_total = weight + weight_new;
    voxel.bump_weights_(y, x) = weight_total;
    voxel.bump_img_(y, x) = (bump_old * weight + weight_new * p_O(2)) / weight_total;
    voxel.intensity_img_(y, x) = (intensity_old * weight + weight_new * p(4)) / weight_total;
    changed(y, x) = 1;
  }
}

void COINBIEVRMap::dilateMask(const Eigen::MatrixXi& changed, const Eigen::MatrixXf& weights,
                               Eigen::MatrixXi& changed_dilated) {
...
void COINBIEVRMap::maskedGaussianSmooth(const Eigen::MatrixXf& image, const Eigen::MatrixXf& weights,
                                         const Eigen::MatrixXi& changed,
                                         Eigen::MatrixXf& image_smooth) {
  int rows = image.rows();
  int cols = image.cols();
...
void COINBIEVRMap::computeScore(Voxel& voxel) {
  int total_count = 0;
...
void COINBIEVRMap::computeIntensityInformation(Voxel& voxel) {
  voxel.intensity_information_.setZero();
  const auto& image = voxel.intensity_img_;
  const auto& weights = voxel.bump_weights_;

  for (int y = 0; y < image.rows(); ++y) {
    for (int x = 1; x + 1 < image.cols(); ++x) {
      if (weights(y, x - 1) > 0.0f && weights(y, x + 1) > 0.0f) {
        voxel.intensity_information_.x() +=
            0.5 * std::abs(image(y, x + 1) - image(y, x - 1));
      }
    }
  }
  for (int y = 1; y + 1 < image.rows(); ++y) {
    for (int x = 0; x < image.cols(); ++x) {
      if (weights(y - 1, x) > 0.0f && weights(y + 1, x) > 0.0f) {
        voxel.intensity_information_.y() +=
            0.5 * std::abs(image(y + 1, x) - image(y - 1, x));
      }
    }
  }
}

const Voxel* COINBIEVRMap::getVoxel(size_t hash_idx) const {
...
Eigen::Vector3i COINBIEVRMap::getVoxelIdx(const Eigen::Vector3d& point) const {
  Eigen::Vector3i idx;
...
Eigen::Vector3d COINBIEVRMap::getVoxelOrigin(const Eigen::Vector3d& point) const {
...
bool COINBIEVRMap::nearestVoxel(const Eigen::Vector3d& point, size_t& result) const {
